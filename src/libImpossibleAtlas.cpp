...
ImageAtlas::ImageAtlas(const std::string& filename, bool debugMode)
{
    ...
    {
        atlasChars.clear();
        this->loadXML(filename, debugMode);
    }
    else
    {
        this->loadBin(atlasChars, debugMode);
    }
}
...
void ImageAtlas::loadBin(std::vector<unsigned char> atlasChars, bool debugMode)
{
    ...
    else
    {
        ...
        for(int i = 0; i < this->imagesArrLen; i++)
        {
            ...
            if(debugMode){std::cout << "Loaded " << tempImg.FragmentArr.size() << " fragment(s)!" << std::endl;}
        
            this->ImagesArr.push_back(tempImg);           
        }

        if(debugMode){std::cout << "Loaded " << this->ImagesArr.size() << " images!" << std::endl;}
        this->imagesArrLen = this->ImagesArr.size();
    }
...
void ImageAtlas::saveToBin(const std::string& filepath)
{
    ...
    writeJavaShort(dataOut, this->imagesArrLen);
...
void ImageAtlas::removeImageByName(std::string_view name)
{
    for(int i = 0; i < this->imagesArrLen; i++)
    {
        if(this->ImagesArr[i].name_imageType_0 == name)
        {
            ...
            for(int j = i; j < this->imagesArrLen - 1; j++)
            {
                this->ImagesArr[j] = this->ImagesArr[j+1];
            }
            this->ImagesArr[this->imagesArrLen] = temp;
...
{   
    ...
    for(int j = index; j < this->imagesArrLen - 1; j++)
    {
        this->ImagesArr[j] = this->ImagesArr[j+1];
    }
    this->ImagesArr[this->imagesArrLen] = temp;
